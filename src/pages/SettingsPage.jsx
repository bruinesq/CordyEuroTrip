...
export default function SettingsPage({ currentUser, travelers, tripName, onTripNameChange, onReload, refreshUser }) {
  ...
  const [pendingAction, setPendingAction] = useState(null) // 'rename_trip' | 'rename_user'
    ...
    if (next.length === 4) {
      setTimeout(() => {
        if (next === MASTER_PIN) {
          ...
          // open the action directly — no session memory
          if (pendingAction === 'rename_trip') setNewTripName(tripName)
          // pendingAction stays set so the action sheet opens
        } else {
    ...
    return (
      <button
        ...
        style={{
          ...
          borderBottom: last ? 'none' : '1px solid var(--warm-100)',
    ...
    <>
      ...
      <div className="card" style={{ padding: 0, marginBottom: 20 }}>
        <SettingRow
          icon="ti-trash"
          label="Clear my data"
          sub="Remove all your flights, expenses and hotel entries"
          danger
          last
          onPress={() => { setShowClearConfirm(true); setClearStep(1) }}
        />
      </div>

      {/* Trip management — master PIN required */}
      ...
      <div style={{ background: 'var(--warm-100)', borderRadius: 10, padding: '8px 12px', marginBottom: 10, display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <span style={{ fontFamily: 'Syne, sans-serif', fontSize: 12, color: 'var(--warm-600)', fontWeight: 600 }}>
          These actions require the master PIN — use when starting a new trip.
        </span>
      </div>
      <div className="card" style={{ padding: 0, marginBottom: 20 }}>
        <SettingRow
          ...
          sub={tripName}
          color="var(--cardinal)"
          onPress={() => openMasterPin('rename_trip')}
        />
        <SettingRow
          icon="ti-user-edit"
          label="Rename a traveler"
          sub="Update any user's display name"
          color="var(--cardinal)"
          last
          onPress={() => openMasterPin('rename_user')}
        />
      </div>
      ...
      {showMasterPin && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 100, background: 'rgba(61,46,30,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={e => e.target === e.currentTarget && cancelMaster()}>
          <div style={{ background: 'var(--cream)', borderRadius: 20, padding: '28px 24px', width: 300, textAlign: 'center' }}>
            ...
            {masterError && (
              <div style={{ color: 'var(--red-err)', fontSize: 13, marginBottom: 10, fontFamily: 'Syne, sans-serif', fontWeight: 600 }}>{masterError}</div>
            )}
            ...
            <button onClick={cancelMaster} style={{ fontFamily: 'Syne, sans-serif', fontSize: 13, color: 'var(--warm-500)', background: 'none', border: 'none', cursor: 'pointer', fontWeight: 600 }}>
              Cancel
            </button>
          </div>
        </div>
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 100, background: 'rgba(61,46,30,0.5)', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }} onClick={e => e.target === e.currentTarget && setPendingAction(null)}>
          <div style={{ background: 'var(--cream)', borderRadius: '22px 22px 0 0', padding: '20px 18px', paddingBottom: 'calc(40px + env(safe-area-inset-bottom,16px))' }}>
            ...
            <div style={{ fontFamily: 'Syne, sans-serif', fontSize: 12, color: 'var(--warm-500)', marginBottom: 14 }}>
              This updates the trip name shown on all devices instantly.
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 8 }}>
              <button onClick={() => setPendingAction(null)} style={{ padding: '14px', background: 'var(--warm-100)', color: 'var(--warm-800)', border: 'none', borderRadius: 13, fontFamily: 'Syne, sans-serif', fontSize: 14, fontWeight: 700 }}>
                Cancel
              </button>
              <button className="kp-submit" style={{ margin: 0 }} onClick={saveTripName}>
                Save
              </button>
            </div>
          </div>
          ...
          <div style={{ background: 'var(--cream)', borderRadius: '22px 22px 0 0', padding: '20px 18px', paddingBottom: 'calc(40px + env(safe-area-inset-bottom,16px))' }}>
            ...
            {!editingUser ? (
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 14 }}>
                  {travelers.map(t => {
                    ...
                    return (
                      <button
                        key={t.id}
                        onClick={() => { setEditingUser(t); setNewUserName(t.name) }}
                        style={{ padding: '12px 10px', background: th.bg, border: 'none', borderRadius: 12, display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}
                      >
                        <div style={{ fontFamily: 'Syne, sans-serif', fontSize: 18, fontWeight: 800, color: th.text, flexShrink: 0 }}>{initials(t.name)}</div>
                        <div style={{ fontFamily: 'Syne, sans-serif', fontSize: 12, fontWeight: 700, color: th.text, textAlign: 'left' }}>{t.name.split(' ')[0]}</div>
                      </button>
                    )
                  })}
                </div>
                <button onClick={() => setPendingAction(null)} style={{ width: '100%', padding: '12px', background: 'none', border: 'none', fontFamily: 'Syne, sans-serif', fontSize: 13, color: 'var(--warm-500)', cursor: 'pointer', fontWeight: 600 }}>
                  Cancel
                </button>
              </>
            ) : (
              <>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 8 }}>
                  <button onClick={() => setEditingUser(null)} style={{ padding: '14px', background: 'var(--warm-100)', color: 'var(--warm-800)', border: 'none', borderRadius: 13, fontFamily: 'Syne, sans-serif', fontSize: 14, fontWeight: 700 }}>
                    Back
                  </button>
                  <button className="kp-submit" style={{ margin: 0 }} onClick={saveUserName} disabled={saving}>
            ...
            {clearStep === 1 && (
              <>
                <div style={{ textAlign: 'center', marginBottom: 20 }}>
                  ...
                  <div style={{ fontFamily: 'Syne, sans-serif', fontSize: 17, fontWeight: 800, color: 'var(--red-err)', marginBottom: 10 }}>
                    Clear all my data?
                  </div>
                  <div style={{ fontFamily: 'Syne, sans-serif', fontSize: 13, color: 'var(--warm-700)', lineHeight: 1.6 }}>
            ...
            {clearStep === 2 && (
              <>
                <div style={{ textAlign: 'center', marginBottom: 20 }}>
                  ...
                  <div style={{ fontFamily: 'Syne, sans-serif', fontSize: 13, color: 'var(--warm-700)', lineHeight: 1.6 }}>
                    The court has reviewed your motion. The verdict is final. All data entered by <strong>{currentUser?.name?.split(' ')[0]}</strong> will be permanently deleted. Not
